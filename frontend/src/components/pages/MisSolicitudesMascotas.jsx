...
function MisSolicitudesMascotas() {
                ...
                {filteredItems.length === 0 ? ( // Verifica si no hay items
                    <div className="flex justify-center items-center h-64">
                        <h2 className="text-center text-2xl font-bold text-red-500">
                            No tienes solicitudes de mascotas disponibles
                        </h2>
                    </div>
                ) : (
    ...
    const peticionGet = async () => {
        try {
            ...
            const fk_id_usuario_adoptante = userInfo.id_usuario; // Extrae el ID del usuario
    
            // Realiza la petición GET al backend con el ID del usuario adoptante
            const response = await axiosClient.get(`/adopciones/proceso/${fk_id_usuario_adoptante}`);
    ...
    const handleDenegarSolicitud = async (id_adopcion) => {
        try {
            await axiosClient.post(`/adopciones/administrar/${id_adopcion}`, {
                accion: 'denegar'
            });
            ...
            Swal.fire({
                position: "center",
                icon: "success",
                title: `Has cancelado tu solicitud de adopción`,
                showConfirmButton: false,
                timer: 1500
            });
    
            // Filtra manualmente la mascota denegada del estado local
            setMascotas(prevMascotas => prevMascotas.filter(mascota => mascota.id_adopcion !== id_adopcion));
    
            // Puedes llamar a peticionGet si quieres asegurarte de que los datos son correctos desde el backend
            // peticionGet();
        } catch (error) {
            console.log('Error denegando la solicitud: ', error);
        }
    };
...
export default MisSolicitudesMascotas
